 ...
 * 
 * (c) 2009-2010 Torstein HÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¸nsi
 * 
...
var logger = org.slf4j.LoggerFactory.getLogger(com.theatlantic.highcharts['export'].SVGHighchartsHelper().getClass());
 ...
 */
function extend(target, b, deep) {
    var src, name, copy, clone, copyIsArray;
	...
	for (name in b) {
	    ...
	    if (deep && copy && (isPlainObject(copy) || (copyIsArray = isArray(copy)))) {
	        ...
	        target[name] = extend(clone, copy, true);
    ...
    if (element && typeof(element) == 'object') {
        if (element.childNodes) {
            for (var i = 0; i < element.childNodes.length; i++) {
                discardElement(element.childNodes[i]);
            }
        }
...
defaultOptions = extend({}, origDefaultOptions, true);
...
var origDefaultPlotOptions = extend({}, origDefaultOptions.plotOptions, true);
var origDefaultSeriesOptions = extend({}, origDefaultPlotOptions.line, true);
...
SVGElement.prototype = {
	...
	destroy: function() {
		...
		/* 
		 * HACK DO NOT DELETE WRAPPER MEMBERS
		 * due to a Rhino bug :
		 * 		'delete' removes key form the wrapper instance AND from the wrapper prototype !!!
		 * 		so no SVGElement could be instanciate after that
		 */
		 var keyCount = 0;
 		 var lastKey;
		 for (key in wrapper) {
...
function Chart (options, callback) {
	...
	function initSeries(options) {
		...
		if (typeof serie.init != 'function') {
		    throw "typeClass for type " + type + " has no init.";
		}
	...
	function recursiveDelete(obj) {
	    for (k in obj) {
	        if (obj[k] && isPlainObject(obj[k])) {
	            recursiveDelete(obj[k]);
	        }
	        delete obj[k];
	    }
	}
	...
	chart.recursiveDelete = recursiveDelete;
	chart.get = get;
...
Point.prototype = {
	...
	destroy: function() {
		...
		each(['graphic', 'tracker', 'group', 'dataLabel', 'connector'], function(prop) {
			if (point[prop] && point[prop].destroy) {
				point[prop].destroy();
			}
		});		
